...
BezierCurve.fromPoints = function (points) {
...
BezierCurve.prototype.getPointAtT = function(t) {
  return {
...
BezierCurve.prototype.getXYTangentParameters = function(epsilonLength) {
  var deriv = this._getDerivative();
  return deriv._getTangentParameters();
};

BezierCurve.prototype._getDerivative = function() {
  return new BezierCurve(
...
BezierCurve.prototype._getTangentParameters = function(epsilonLength) {
  ...
  for (; i < n; i++) {
    t = ts[i];
    tPrev = ts[i - 1];
    if (!this._nearlySamePointParameters(tPrev, t, epsilonLength)) {
      mergedTs.push(t);
    }
  }
  return mergedTs;
...
BezierCurve.prototype._nearlySamePointParameters = function(t1, t2, epsilonLength) {
  var p1 = this.getPointAtT(t1);
  var p2 = this.getPointAtT(t2);
...
BezierCurve.prototype.getCurveFromZeroToT = function(t) {
  var xs = this.xs;
  var ys = this.ys;
  var p0 = {x: xs[0], y: ys[0]};
  var p01t = new BezierCurve([xs[0], xs[1]], [ys[0], ys[1]]).getPointAtT(t);
  var p12t = new BezierCurve([xs[1], xs[2]], [ys[1], ys[2]]).getPointAtT(t);
  var q2 = new BezierCurve.fromPoints([p01t, p12t]).getPointAtT(t);
  var pt = this.getPointAtT(t);
  return BezierCurve.fromPoints([p0, p01t, q2, pt]);
};
